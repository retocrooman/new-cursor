...
import { findTaskById, TaskFeatureError } from "@new-cursor/tasks-feature";
...
export async function createVerifyRunFromCompleted(
  tx: DbOrTx,
  input: { message: DeliveryMessage; agentId: string; fanOutIndex: number },
): Promise<PendingRunExecution | null> {
  const payload = runCompletedPayloadSchema.parse(input.message.payload);
  if (payload.status !== "completed" || payload.stage !== "verify") {
    return null;
  }
